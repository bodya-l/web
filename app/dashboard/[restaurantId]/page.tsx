...
// Типи, що відповідають даним Pusher
type ItemDetails = {
  name: string;
  quantity: number;
  price: number;
...
export default function RestaurantDashboard() {
  ...
  // 1. Завантаження існуючих замовлень (Заглушка)
  useEffect(() => {
    if (!restaurantId) return;
    // В реальності: fetch(`/api/orders/${restaurantId}`).then(...)
    setOrders([]);
    setIsLoading(false); 
  }, [restaurantId]);

  // 2. Налаштування Pusher (Отримання нових замовлень)
  useEffect(() => {
    if (!restaurantId || orders === null) return; 
  ...
  const handleStatusChange = async (orderId: number, currentStatus: Order['status']) => {
      ...
      else if (currentStatus === 'READY') newStatus = 'COMPLETED'; // Видача замовлення
      else return; 
  ...
  if (isLoading) return <div className="p-8 text-center text-gray-500">Завантаження панелі...</div>;
              ...
              <div className="flex-grow py-4 border-y border-gray-100">
                <ul className="space-y-1 text-sm">
                  {order.items.map((item, index) => (
                    // ⬅️ Виправлення відображення: дозволяємо перенесення слів
